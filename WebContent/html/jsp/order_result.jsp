...
String userLevel =(String)session.getAttribute(Constants.USER_LEVEL);
	...
	//out.print(totalRecord + "-" + pageRecord);
...
<%@page import="java.math.BigDecimal"%><table cellspacing="3" border="0" class="main">
        <tbody><tr>
        ...
        <tr>
            <td><div id="pagingDiv">
                    <input type="Button" value="   &lt;&lt;   " id="firstPage" onclick='movePaging("FIRST","pagingMove","order_results");'/>
                    <input type="Button" value="   &lt;    " id="prePage" onclick='movePaging("PREV","pagingMove","order_results");'/>
                    <input type="Button" value="    &gt;   " id="nextPage" onclick='movePaging("NEXT","pagingMove","order_results");'/>
                    <input type="Button" value="   &gt;&gt;   " id="lastPage" onclick='movePaging("LAST","pagingMove","order_results");'/>
                    <input type="Button" value=" Requery " onclick='movePaging("FIRST","pagingMove","order_results");' />
                 </div>
            </td>
        </tr>
  </tbody></table>
 <%}if(listOrder.size()>0){ %> 
<table id="OrderTable" border=0 cellpadding=2 width="100%" class="tablers main">
<tr style="background-color:#CCCCCC">
<td align="right" colspan="15" ><a href="exportOrder.do?method=excelAllOrder"><font color="red"><u>EXPORT TO EXCEL</u></font></a></td>
</tr>
...
<tr style="background-color:#0099FF">
    ...
    <td align=left valign=bottom><b>Status</b></td>
    ...
    <td align=right valign=bottom nowrap="nowrap"><b>System Total</b></td>
...
<%
    ...
    {
        String company =(order.getShip_to_company() != null )? order.getShip_to_company(): "";
    ...
    <td align=left valign=bottom><%=order.getId()%></td>
  	<td align=left valign=bottom><a href="order_db.do?method=orderViewPendingSearch&order_id=<%=order.getOrderId()%>&from=order_result"" ><%=order.getOrderId()%></a></td>
    <td align=left valign=bottom><%=order.getDayOrder()%></td>
    <td align=left valign=bottom>
    	<%
        if(userLevel!=null && !userLevel.equals("C")){
        %>
    	<a href="shopper_manager.do?method=getShopperDetails&shopperId=<%=order.getShopId()%>&from=order_result" ><%=  company.isEmpty() ? order.getShip_to_name() : order.getShip_to_name()+" ("+company+")" %></a>
    	<%}else{ %>
    	<%=  company.isEmpty() ? order.getShip_to_name() : order.getShip_to_name()+" ("+company+")" %>
    	<%} %>
    </td>
    <td align=left valign=bottom><%=order.getStatus() %></td>
	...
	<td align=right valign=bottom><%=Converter.getMoney(order.getDesk().getMhz())%></td>  
     </tr>           
 <%   }%>
</table>
<%}else{ %>
